...
6. `eth_call` / `eth_estimateGas` still execute with an EIP-1559 + zero-fee call context, but now pre-validate conflicting fee fields and reject EIP-1559 `maxFeePerGas < block.baseFeePerGas` [crates/module-system/module-implementations/sov-evm/src/helpers.rs:29].  
   Minimal repro: vary `gasPrice`, `maxFeePerGas`, `maxPriorityFeePerGas`, and `type`; the execution context stays fixed, but below-base-fee EIP-1559 requests now fail before execution.
