...
impl<C: sov_modules_api::Context> TestStruct<C> {
    ...
    pub fn second_method(&self, result: u32, _working_set: &mut WorkingSet<C>) -> RpcResult<u32> {
        Ok(result)
    }
    ...
    pub fn fourth_method(&self, _working_set: &mut WorkingSet<C>, result: u32) -> RpcResult<u32> {
        Ok(result)
    }
}
...
struct RpcStorage<C: Context> {
    pub storage: ::std::sync::Arc<::std::sync::RwLock<C::Storage>>,
}

impl TestStructRpcImpl<ZkDefaultContext> for RpcStorage<ZkDefaultContext> {
    fn get_working_set(&self) -> ::sov_modules_api::WorkingSet<ZkDefaultContext> {
        let storage = {
