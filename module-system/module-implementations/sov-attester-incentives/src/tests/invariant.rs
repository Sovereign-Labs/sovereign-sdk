...
fn test_transition_invariant() {
    ...
    // Process a valid attestation for the first transition *should fail*
    {
        ...
        let attestation = Attestation {
            ...
            da_block_hash: MockHash([(init_height_usize + 1).try_into().unwrap(); 32]),
    ...
    for i in 0..usize::try_from(DEFAULT_ROLLUP_FINALITY + 1).unwrap() {
        let old_attestation = Attestation {
            ...
            da_block_hash: MockHash([(new_height).try_into().unwrap(); 32]),
        ...
        let new_attestation = Attestation {
            ...
            da_block_hash: MockHash([(new_height + i).try_into().unwrap(); 32]),
    ...
    let old_attestation = Attestation {
        ...
        da_block_hash: MockHash([(new_height + finality_usize + 1).try_into().unwrap(); 32]),
