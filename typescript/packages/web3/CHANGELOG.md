...
-   f3578d9: Adds a Signer implementation for Ledger devices with the Solana app, intended to be used with the `SolanaSignableRollup` on rollups that use the `sov-solana-offchain-auth` authenticator crate.
-   f3578d9: Added support for encoding transactions as a solana offchain messages, for rollups that use the corresponding authenticator. Use the `SolanaSignableRollup` and select the desired encoding.
