...
impl<T, Slot, Batch, Tx> LedgerRpcClient<Slot, Batch, Tx> for T
where
    T: SubscriptionClientT,
    Slot: serde::Serialize + DeserializeOwned,
    Batch: serde::Serialize + DeserializeOwned,
    Tx: serde::Serialize + DeserializeOwned,
{
    async fn get_head(&self, query_mode: QueryMode) -> RpcResult<Option<Slot>> {
        self.request("ledger_getHead", Some(query_mode))
            .await
            .map_err(Into::into)
    }
}
...
/// Experiment
#[jsonrpsee::core::async_trait]
pub trait LedgerRpcClient<Slot, Batch, Tx>
where
