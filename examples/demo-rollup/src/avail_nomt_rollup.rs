...
#[derive(Default)]
pub struct AvailNomtDemoRollup<M> {
    phantom: std::marker::PhantomData<M>,
}

type Hasher = <Risc0CryptoSpec as CryptoSpec>::Hasher;
type NativeStorage =
    NomtProverStorage<DefaultStorageSpec<Hasher>, <AvailDASpec as DaSpec>::SlotHash>;

/// The default spec of the rollup
pub type AvailNomtRollupSpec<M> = ConfigurableSpec<
    AvailDASpec,
    Risc0,
    MockZkvm,
    MultiAddressEvm,
...
impl RollupBlueprint<Native> for AvailNomtDemoRollup<Native>
where
    AvailNomtRollupSpec<Native>: PluggableSpec,
    <AvailNomtRollupSpec<Native> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = AvailNomtRollupSpec<Native>;
    type Runtime = Runtime<Self::Spec>;
}

impl RollupBlueprint<WitnessGeneration> for AvailNomtDemoRollup<WitnessGeneration>
where
    AvailNomtRollupSpec<WitnessGeneration>: PluggableSpec,
    <AvailNomtRollupSpec<WitnessGeneration> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = AvailNomtRollupSpec<WitnessGeneration>;
    type Runtime = Runtime<Self::Spec>;
}
