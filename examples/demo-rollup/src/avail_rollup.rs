...
/// Rollup with a [`ConfigurableSpec`] with [`AvailDASpec`] as Da spec, [`Risc0`] inner vm and [`MockZkvm`] for outer vm
#[derive(Default)]
pub struct AvailDemoRollup<M> {
    phantom: std::marker::PhantomData<M>,
}

/// The default spec of the rollup
pub type AvailRollupSpec<M> = ConfigurableSpec<AvailDASpec, Risc0, MockZkvm, MultiAddressEvm, M>;

impl RollupBlueprint<Native> for AvailDemoRollup<Native>
where
    AvailRollupSpec<Native>: PluggableSpec,
    <AvailRollupSpec<Native> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = AvailRollupSpec<Native>;
    type Runtime = Runtime<Self::Spec>;
}

impl RollupBlueprint<WitnessGeneration> for AvailDemoRollup<WitnessGeneration>
where
    AvailRollupSpec<WitnessGeneration>: PluggableSpec,
    <AvailRollupSpec<WitnessGeneration> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = AvailRollupSpec<WitnessGeneration>;
    type Runtime = Runtime<Self::Spec>;
}

#[async_trait]
impl FullNodeBlueprint<Native> for AvailDemoRollup<Native> {
    type DaService = AvailDAService;

    type StorageManager =
        NativeStorageManager<AvailDASpec, <AvailRollupSpec<Native> as Spec>::Storage>;
