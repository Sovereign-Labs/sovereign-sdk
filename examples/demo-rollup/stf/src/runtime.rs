...
use sov_modules_api::{LazyChainHash, RawTx, Spec};
...
fn current_chain_data() -> sov_modules_api::sov_universal_wallet::schema::ChainData {
    ...
    S::Address: FromVmAddress<EthereumAddress> + FromVmAddress<Base58Address> + HyperlaneAddress,
{
    ...
    fn endpoints(
        ...
        // The schema must be loaded from its JSON form: the Borsh form intentionally omits the
        // wallet-facing transaction templates and serde metadata.
        let schema = sov_modules_api::sov_universal_wallet::schema::Schema::from_json(
            __generated::SCHEMA_JSON,
        )
        .expect("Failed to deserialize schema")
        .with_chain_data(current_chain_data());
...
mod tests {
    ...
    fn endpoint_schema() -> sov_modules_api::sov_universal_wallet::schema::Schema {
        sov_modules_api::sov_universal_wallet::schema::Schema::from_json(__generated::SCHEMA_JSON)
            .expect("failed to deserialize generated JSON schema")
            .with_chain_data(current_chain_data())
    }

    #[test]
    fn generated_schema_matches_runtime_chain_hash_template() {
        let schema = borsh::from_slice::<sov_modules_api::sov_universal_wallet::schema::Schema>(
            __generated::SCHEMA_BORSH,
        )
        .expect("failed to deserialize generated schema")
        .with_chain_data(current_chain_data());

        assert_eq!(
            schema.chain_hash().expect("failed to hash runtime schema"),
            runtime_chain_hash(),
            "generated schema and authentication chain hashes disagree"
        );
    }

    #[test]
    fn endpoint_schema_retains_wallet_metadata() {
        assert!(
            !endpoint_schema().serde_metadata().is_empty(),
            "the schema served by /rollup/schema must include serde metadata so wallets can \
             encode JSON transactions to borsh"
        );
    }

    #[test]
    fn endpoint_schema_matches_runtime_chain_hash() {
        assert_eq!(
            endpoint_schema()
                .chain_hash()
                .expect("failed to hash endpoint schema"),
            runtime_chain_hash(),
            "the schema served by /rollup/schema and the authentication chain hash disagree"
        );
    }
}
