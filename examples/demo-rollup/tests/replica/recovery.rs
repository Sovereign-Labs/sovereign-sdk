...
async fn verify_replica_processes_tx(
    ...
    verify_node_processes_tx(leader, replica, key, token_id, receiver_addr, nonce).await;
}

async fn verify_node_processes_tx(
    sender: &TestRollup<Rollup>,
    observer: &TestRollup<Rollup>,
    key: &<<S as Spec>::CryptoSpec as CryptoSpec>::PrivateKey,
    token_id: sov_bank::TokenId,
    receiver_addr: <S as Spec>::Address,
    nonce: u64,
    ...
    let mut event_subscription = observer
        .api_client()
        .subscribe_to_events_with_filter("Bank/*")
        .await
        .unwrap();

    sender.send_tx_to_sequencer(&tx).await.unwrap();
