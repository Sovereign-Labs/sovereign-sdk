...
async fn check_value(client: &demo_stf_json_client::Client, expected: u64) {
    ...
    match &*response {
        RuntimeAnyJsonValue::Object(inner) => {
            ...
            println!("VALUE: {state_value:?}");
            let heavy_vec = state_value.as_array().expect("This state value should be array");
...
/// committed to git and is meant to be reused long-term, only regenerated by manual invocation
...
async fn test_generate_mockda_dataset_for_resync() -> anyhow::Result<()> {
    ...
    let test_rollup = start_rollup(rollup_storage_path.clone()).await?;
...
async fn test_rollup_resync() -> anyhow::Result<()> {
    ...
    let known_acceptable_logs = [
        ...
        // TODO: Should we wipe sequencer DB too, to emulate clean resync?
...
async fn sync_rollup_with_path(
    ...
    tracing::info!("Node has synced, waiting for sequencer to be ready");
    test_rollup.wait_for_sequencer_ready().await?;
    tracing::info!("Sequencer is ready, wait 1 second for stars to align");
    ...
    // Could we get rollup height from sequencer to be 100% sure
    tokio::time::sleep(Duration::from_millis(1000)).await;
    tracing::info!("Synced!");
