...
//!
//! The blueprint resolves the runtime sequencer role up-front via
//! `ResolvedSequencerDb::resolve` and feeds the result into
//! `should_enable_proof_pipeline`, which forces witness generation, prover
//! service creation, and the runner's `proof_manager` arg all to `None` on a
//! replica.
//!
...
//!
//! This test currently runs as a smoke check: it boots master + replica with
//! `proof_manager = Some(...)` on both, waits for the master to produce
//! several aggregated proofs, and confirms the replica syncs them via DA.
//! If the replica's proof pipeline were accidentally re-enabled, the test
//! would still pass — sharper assertions (one of the options above) are the
//! follow-up to bolt on.
