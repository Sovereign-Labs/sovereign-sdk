...
async fn denied() -> anyhow::Result<()> {
    ...
    assert_eq!(err.to_string(), "server returned an error response: error code -32003: Contract creation is only allowed from allowed addresses. 0x3FE0233e6cf3c9753fcB7449987EC49C88aDDE71 is not on the list");
    Ok(())
}
