...
async fn simple_transfer() -> anyhow::Result<()> {
    ...
    assert_eq!(gas_estimation, (9778 / 2) * 3 + 100_000); // Simple transfer consumes 0 EVM gas so we only see the ABSOLUTE_MARGIN
    Ok(())
}
...
async fn contract_deploy() -> anyhow::Result<()> {
    ...
    assert_eq!(gas_estimation, (243_106 / 2) * 3 + 100_000);
    Ok(())
}
