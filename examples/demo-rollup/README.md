...
To generate transactions you must use the `sov-cli generate-transaction` subcommand, as shown below:

```
$ ./target/debug/sov-cli generate-transaction -h
Serialize a call to a module. This creates a .dat file containing the serialized transaction

Usage: sov-cli generate-transaction <SENDER_PRIV_KEY_PATH> <MODULE_NAME> <CALL_DATA_PATH> <NONCE>
...
```
$ ./target/debug/sov-cli generate-transaction ./examples/test-data/keys/minter_private_key.json Bank ./examples/test-data/requests/transfer.json 0
```
