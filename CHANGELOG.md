# 2026-07-03
- #3028 **Breaking Change** `sov-modules-api` no longer reads the chain-identity constants (`CHAIN_ID`, `CHAIN_NAME`, `CHAIN_HASH_OVERRIDES`) at macro-expansion time, so editing `chain-metadata.toml` no longer rebuilds the module system: only the crates that actually read chain identity (the runtime/stf crate, `sov-evm`, `sov-rollup-apis`, `sov-ethereum`, auth glue, tools, binaries) recompile on a network switch. Chain values now flow through the `Runtime` trait, which gains two required methods — `fn chain_id() -> u64` and `fn chain_hash_overrides() -> &'static [ChainHashOverride]` — that downstream runtimes implement as one-liners reading `config_value!("CHAIN_ID")` / `config_value!("CHAIN_HASH_OVERRIDES")`. By design no public API accepts a loose `expected_chain_id`-style parameter (a stale caller-supplied value would silently weaken the chain-id defense-in-depth check under the chain-hash binding); values travel only via `Rt: Runtime<S>` generics, except build-time schema generation which takes explicit `ChainData`. Migrations: `config_chain_id()` removed → `Rt::chain_id()`; `resolve_chain_hashes_for_height()` removed → `resolve_chain_hashes(height, Rt::chain_hash_overrides(), default_hash)`; `verify_and_decode_tx` removed (no callers; use `authenticate` or a local eip712-style flow); `authenticate` is now generic over `Rt: Runtime<S>` and no longer takes `default_chain_hash`; `verify_chain_id` is generic over `Rt`; `get_runtime_schema` and `sov_build::Options::apply`/`apply_defaults` take an explicit `ChainData` (re-exported as `sov_build::ChainData`; construct it in the rollup's build script from `config_value!`); `StandardSchemaEndpoint::new` takes the runtime's `chain_hash_overrides()`; `SovApiProofSender::new` and the proof-blob serialize helpers take an explicit `chain_id`; the CLI `--chain-id` argument is now optional and defaults to the runtime's configured chain id (filled by the wallet layer). Not state-breaking; transaction verification behavior is unchanged.
- #3028 demo-rollup: the `sov-demo-rollup` package no longer recompiles on every warm build. Its build script declared `cargo:rerun-if-changed=NULL` (since #1411, when the script still ran `sov_build` codegen with a baked git hash); Cargo treats a watched path that doesn't exist as permanently stale, so the script — and with it the whole package (lib + all bins, ~8 s) — reran on every build. The script's only remaining inputs (`SKIP_GUEST_BUILD`, `SOV_PROVER_MODE`) were already declared via `rerun-if-env-changed`, so the dangling watch is simply removed. Example crate only; no behavior change beyond rebuild avoidance.
- #3028 Build: editing `constants.toml` no longer recompiles the `sov-modules-macros` proc-macro crate itself, avoiding the proc-macro dependency fanout that previously dirtied nearly every crate using Sovereign macros. The build script used to declare `cargo:rerun-if-changed=<constants.toml>`, so any content edit dirtied the proc-macro crate and Cargo conservatively recompiled its dependents. Constants are now tracked from expanded code instead: `config_value!` / `config_value_private!` / `#[gas]`-field expansions embed `include_bytes!(<manifest path>)`, which registers the file in the dep-info of crates that read constants. Crates downstream of those consumers may still rebuild; for example, `sov-modules-api` reads `ADDRESS_PREFIX`, so changing the manifest can still dirty `sov-modules-api` and then crates that depend on it. Also fixes `CONSTANTS_MANIFEST` and `SOV_TEST_MODE_CONST_MANIFEST` not being declared `rerun-if-env-changed` (changing them did not re-resolve the manifest path), and improves the macro error when the baked manifest path is no longer readable (e.g. after moving the workspace). Not state- or API-breaking; expansion output gains an anonymous `const _: &[u8]` alongside each constant.
- #3028 adds an optional `chain-metadata.toml` manifest tier for per-network chain identity. The five chain-identity constants — `CHAIN_ID`, `CHAIN_NAME`, `CHAIN_HASH_OVERRIDES`, `BATCH_NAMESPACE`, `PROOF_NAMESPACE` — can now live in a `chain-metadata.toml` file next to `constants.toml` (`chain-metadata.testing.toml` next to `constants.testing.toml`; same `[constants]` format). Routing is static, by key name: these five keys — and only these — are read from (and dependency-tracked against) the chain-metadata file when it exists, so editing them on a network switch recompiles only the crates that read them. All other keys, including `[gas]` tables and module discriminants, stay in `constants.toml`. Non-breaking: when `chain-metadata.toml` doesn't exist, chain keys are read from `constants.toml` as before. To migrate, move the five keys to the new file and remove them from `constants.toml` (the removal triggers the one-time rebuild that picks up the new file); once the file exists, a chain key missing from it is a compile error, even if `constants.toml` still defines it — a partial migration fails loudly instead of silently tracking both files. Standalone effect: none yet — tracking is per file, and `sov-modules-api` still reads `CHAIN_ID`/`CHAIN_NAME`/`CHAIN_HASH_OVERRIDES` at expansion time, so it tracks `chain-metadata.toml` and any edit to that file (namespaces included) still rebuilds the workspace minus `sov-modules-macros` (measured: 35 units). The payoff lands with the follow-up that removes those reads from `sov-modules-api`: chain-metadata edits will then rebuild only the upper-layer crates that read chain identity. The Hive Dockerfile now rewrites `CHAIN_ID` in the chain-metadata files.
