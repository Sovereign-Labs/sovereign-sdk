...
# 2026-06-02
- #2886 Re-enables NOMT state-version pruning (disabled since the rockbound type-aware refactor) and **changes the pruning config (breaking)**: the flat `storage.pruner_block_interval` / `pruner_versions_to_keep` / `pruner_max_batch_size` fields are replaced by a single `storage.pruner` enum. Modes: omit the section (or `pruner = "off"`) to disable pruning (the default); `[storage.pruner.periodic]` with `block_interval` + `versions_to_keep` (+ optional `max_batch_size`) for the previous interval-driven background pruning; and `[storage.pruner.once_at_startup]` with `versions_to_keep` (+ optional `max_batch_size`, `compact_after`) to prune synchronously to completion once at node startup — before block processing begins — and then never again that run, so the pruning I/O does not compete with live read/write traffic. With `compact_after = true`, the accessory state column family and the user/kernel archival historical + pruning column families are compacted after the startup prune to drop tombstones and reclaim disk (heavy; off by default). Existing TOML configs must move to the new `[storage.pruner...]` form: `storage` (and the `pruner` enum) now use `#[serde(deny_unknown_fields)]`, so the deprecated flat `pruner_*` keys — and any other unknown `[storage]` or `[storage.pruner.*]` key — are **rejected at config-parse time** instead of being silently ignored and leaving pruning disabled. `versions_to_keep` and `max_batch_size` are non-zero integer types, so a `0` is likewise rejected at parse time. Hardens the native unversioned read path in `sov-state`: unbound NOMT reads now log and return `None` on a transient I/O fault instead of `.expect()`-panicking (by construction they can never observe a pruned version), and the rare pruned-version-race panic on a stale `Storage::get` reader is now preceded by a `tracing::warn!` so it is observable in production logs. The mockda→celestia migration default-enables periodic pruning for migrated nodes only when no `[storage.pruner]` section is present (an explicit `pruner = "off"` is respected). Bumps the `rockbound` dependency to rev `0aa43d4e581e047ba8d8e7a662ec6ade32b2cae3`, which owns the historical/pruning column families and exposes the `collect_pruning_batch` API the pruner uses, makes the user/kernel pruning emit a range-delete tombstone for the pruning column family (instead of per-key point deletes), groups pruning entries by key so each pruned key needs at most one historical-CF lookup per pass, adds `VersionedDB::trigger_compaction` so `compact_after` compacts the user/kernel archival CFs, and makes `SchemaBatch::merge` preserve range operations (splitting earlier range tombstones around later point writes so last-write-wins holds across a merge).
