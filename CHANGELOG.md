...
- #2004 The sequencer will now buffer and intelligently reorder transactions with a nonce that arrive out-of-order within a short window of time. Adds `max_future_nonce_delta` and `future_nonce_transaction_timeout_millis` optional config options that allow configuring the limits of how eagerly the sequencer will try to buffer nonces.
  ...
  - **Breaking change** Custom sequencer transaction acceptance endpoints (which are used for accepting transactions with non-standard authenticators) should remove the `tokio::spawn()` wrapping the sequencer `accept_tx()` call, if there is one. It was previously used to shield `accept_tx()` from API cancellation, as the internal sequencer logic is not safe; but this has now been moved internally into the sequencer, and the `accept_tx()` function is now cancellation-safe and can be used directly inside API handlers.
