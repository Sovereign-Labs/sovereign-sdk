# 2026-07-03
- #PR_NUMBER Build: editing `constants.toml` no longer recompiles the `sov-modules-macros` proc-macro crate (and with it the crates that don't read constants). The build script previously declared `cargo:rerun-if-changed=<constants.toml>`, so any content edit dirtied the proc-macro crate and Cargo conservatively recompiled every dependent crate. Constants are now tracked per consuming crate instead: `config_value!` / `config_value_private!` / `#[gas]`-field expansions embed `include_bytes!(<manifest path>)`, which registers the file in the dep-info of exactly the crates that read it. Also fixes `CONSTANTS_MANIFEST` and `SOV_TEST_MODE_CONST_MANIFEST` not being declared `rerun-if-env-changed` (changing them did not re-resolve the manifest path), and improves the macro error when the baked manifest path is no longer readable (e.g. after moving the workspace). Not state- or API-breaking; expansion output gains an anonymous `const _: &[u8]` alongside each constant.
