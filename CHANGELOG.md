...
# 2025-09-11
- #1664 **BREAKING CHANGE** A new method has been added to the `Runtime` trait: `fn schema() -> &'static Schema`. This should return the UniversalWallet schema for this runtime. This is currently used by the EIP-712 transaction authenticator.
The demo-rollup implementation provides an example for how to do this when generating the schema using the standard `sov-build` buildscript for your rollup. If you are not currently generating a schema for your rollup and do not need any features relying on it, you can use `Default::default()` for the initialization in the trait.
