...
# 2026-06-23
- #PR_NUMBER REST API: Fix single-item lookups (`GET .../state/{map}/items/{key}`) for StateMaps with composite (multi-field struct) keys. The handler now parses the key path segment via `FromStr` (the key's `Display` form, e.g. `route_id:remote_domain`) instead of serde deserialization, which previously failed with HTTP 400 `missing field ...`. Simple keys are unaffected. Keys whose string form contains `/` (spanning multiple path segments) remain unsupported.
