# 2026-06-02
- #PR_NUMBER **Breaking config change**: Replaces the flat `storage.pruner_block_interval` / `pruner_versions_to_keep` / `pruner_max_batch_size` fields with a single `storage.pruner` enum. Modes: omit the section (or `pruner = "off"`) to disable pruning (the default); `[storage.pruner.periodic]` with `block_interval` + `versions_to_keep` (+ optional `max_batch_size`) for the previous interval-driven background pruning; and `[storage.pruner.once_at_startup]` with `versions_to_keep` (+ optional `max_batch_size`, `compact_after`) to prune synchronously to completion once at node startup — before block processing begins — and then never again that run, so the pruning I/O does not compete with live read/write traffic. With `compact_after = true`, the accessory state column family is compacted after the startup prune to drop tombstones and reclaim disk (heavy; off by default). Existing TOML configs must move to the new `[storage.pruner...]` form. Follow-up: the user/kernel historical pruning still emits point-delete tombstones inside `rockbound` and is not yet compacted on startup — both are tracked for a `rockbound`-side change (a `VersionedDB` range-delete + compaction helper).
