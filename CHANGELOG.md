# 2026-09-04
- #PR_NUMBER Authentication: The standard authenticator accepts V0 transactions in the pre-fork wire format (no `address_override`, pre-fork signing payload, `chain_id` in place of `chain_hash_fragment`), authenticating them with the pre-fork rules (`CHAIN_ID` check, signature tried against every chain hash valid at the execution height). This lets clients lag behind the hard fork upgrade. Legacy envelopes are also decoded by `decode_sov_tx` for the sequencer's decode path.
...
* A redesign of `sov-accounts` for improved account abstraction and authorization management. Rather than mapping signing data to accounts one-to-many, instead signing credentials can have explicit authorization set or revoked for any account (many-to-many), and transactions can explicitly specify a target account to execute as. A credential's default address can also now have authorization explicitly revoked.
* An improved transaction format which reduces malleability of multisig transactions, resulting in more predictable UX and eliminating some safety footguns.
* A fix for a hash malleability issue with Generation uniqueness used in combination with V1 (multisig) transactions.
* A limit on generation and window-based uniqueness nonce growth, rejecting values larger than 2x the last used generation/nonce as a sanity check against user error (as these mechanisms are ratcheting, and unlimited increases made it trivial to accidentally burn an account).
* On-chain tracking of a state version across hard forks, together with a library for the offline state migrations between versions.
* A refactor of custom sequencing data support, which among other things makes correct implementation of custom EVM precompiles much simpler for developers.

The transaction wire format, the signing payloads, and the chain hash all change. Signatures produced before the fork are no longer valid (in-flight multisigs must be re-signed after the upgrade). Upgrading requires adapting rollup and client code to the breaking changes below, an operational upgrade with a state migration, and upgraded client SDKs to sign transactions in the new format: the TypeScript `@sovereign-sdk/*` packages (see the `@sovereign-sdk/multisig` README and the package changesets for migration notes) and the Python bindings.

### Operating across a hard fork
Major hard forks of Sovereign SDK require operating separate rollup binaries pre- and post-fork. The rollup blueprint accepts `stop_at_rollup_height` and `start_at_rollup_height` parameters, conventionally exposed through the CLI as `--stop-at-rollup-height` and `--start-at-rollup-height`; to upgrade, a fork height (in the future) must be selected, and the old rollup binary restarted with the upgrade height passed in. State migrations must then be run at this point to prepare for the new version. The new rollup binary can then be started from the corresponding start block. To automate this, we provide the [`sov-rollup-manager`](https://github.com/Sovereign-Labs/sov-rollup-manager) utility which wraps the running rollup process, and takes a list of paths to binaries and the configured start-stop heights as configuration.

The upgrade height becomes a property of consensus, and both binaries are subsequently required to resync the rollup from genesis.

Concretely, for this fork (state version 0 -> 1):
* #2856 #2864 #2938: Chain state now records a `state_version`, which must match the `STATE_VERSION` constant compiled into the binary — on mismatch the rollup refuses to start. Pre-fork state is implicitly version `0`; post-fork binaries must set `STATE_VERSION = 1` in constants.toml. On fresh chains, the chain-state genesis config now requires a `state_version` field (set it to the binary's version); on existing chains, migrations bump the stored version.
* #2864 #2965: The migration for this fork is `sov_migrations::v1` in the new `sov-migrations` crate. It is a library — each rollup wraps it in a small binary; see demo-rollup's `migrate_to_v1` (`--rollup-config-path`, `--db-path`, `--dry-run`) for the reference implementation. It drains the legacy credential-to-address map into the new `account_owners` authorization map and bumps the stored state version. NOMT storage only. It is idempotent: re-running it against an already-migrated database is a safe no-op.
* **If the rollup has ever accepted V1 (multisig) transactions**, the stored generation of every account that submitted one must additionally be incremented by `PAST_TRANSACTION_GENERATIONS` (5000 by default) to invalidate pre-fork transaction hashes (see the Generation malleability fix below). This step is not included in `sov_migrations::v1` and must be scripted by the operator. No action is needed if there are no such accounts.
* constants.toml delta: `STATE_VERSION` is added; `INITIAL_GAS_LIMIT`/`UPDATED_GAS_LIMIT`/`CHANGE_GAS_LIMIT_AFTER_HEIGHT` are replaced by a single `BLOCK_GAS_LIMIT`; `EVM_RECEIPT_ACTUAL_FEE_HEIGHT` is removed; the per-type borsh gas constants (`{DEFAULT,TX,PROOF,STRING}_*_BORSH_DESERIALIZATION`) are replaced by `BORSH_PER_BYTE_READ` and `BORSH_PER_READ_BIAS` (`BIAS_BORSH_DESERIALIZATION` remains).

### Accounts and authorization
- #2770 #2771 **Breaking change (code, wire, consensus)**: Credential authorization is now stored as an explicit (address, credential) map, and V0/V1 transactions gain a signed optional `address_override` field selecting the account to execute as. `None` resolves to the credential's default address, which is admitted unless explicitly revoked; `Some(addr)` requires an explicit authorization for that (address, credential) pair, otherwise the transaction is skipped pre-execution (the user is not charged). Context resolution no longer writes state: `InsertCredentialId` records an explicit authorization for the sender's address but no longer re-routes the credential's default address — acting as another account is done per-transaction via `address_override`. Removed native APIs: `Accounts::resolve_sender_address{,_read_only}` and `get_account`; use the new `is_explicitly_authorized` / `is_default_address_authorized` / `is_authorized_for` capabilities. Genesis `accounts` entries now create explicit authorizations, and genesis rejects addresses that correspond to a real keypair where this is checkable (only synthetic addresses may be pre-registered).
- #2773 #2850 **Breaking change (code, wire, consensus)**: `sov_accounts::CallMessage` is now generic in `S: Spec` and gains four variants: `AddCredentialToAddress`, `RemoveCredentialFromAddress`, and `RotateCredentialOnAddress` (all three require `context.sender() == address`; rotate is a remove+add collapsed into one call for key rotation; there is no orphan guard on remove), plus `CreateSyntheticAddress { salt }`, which registers an address with no canonical-signer derivation (suitable for e.g. DAO/treasury accounts). The synthetic address is derived from `(domain || visible_slot_hash || sender || sender_credential || salt)` and the caller's credential is authorized for it; re-issuing the same `(caller, salt)` in the same visible slot is an idempotent no-op. Note this is *bind-then-use*, not *counterfactual*: because the visible slot hash is part of the preimage, the address cannot be predicted and prefunded ahead of the creating transaction (unlike CREATE2 / ERC-4337). The module now emits `CredentialAdded` / `CredentialRemoved` / `CredentialRotated` / `SyntheticAddressCreated` events (`sov_accounts::Module::Event` changes from `()` to `Event<S>`).
- #2853 Accounts REST: `GET /modules/accounts/authorizations/{address}/{credential_id}` gains `admit_as_override` and `admit_as_default` fields, mirroring the two on-chain admit paths above. The pre-existing `authorized` field is deprecated — its canonical-fallback semantic can disagree with the chain for VM credentials (notably EVM); read `admit_as_default` instead. Additive.
- #3043 Sequencer APIs: recent (cached) transactions in the REST API and the websocket transaction subscription now include the submitting `credential_id`. Additive; transactions already evicted from the sequencer cache into the ledger do not carry the field.

### Transaction format and signing
- #2688 **Breaking change (code, wire, consensus)**: The transaction format is versioned and changed to fix Credential ID malleability that allowed the same signed bytes to be replayed for different credentials — across V0 and V1 (multisig) transactions, and with different V1 multisig parameters — a footgun that could cause security issues if accounts were not separated properly. Multisig transactions now explicitly include the multisig parameters in the signed bytes; with the malleability eliminated, signing key reuse across different multisigs is no longer a concern.
- #2876 **Breaking change (code, client code)**: The transaction signing preimage types have been renamed to `TransactionSigningPayload{,V0,V1}` and now include the chain hash directly. `UnsignedTransaction` is now the user-created payload containing only the runtime call, uniqueness data, transaction details, and optional address override. For creating a multisig, use `UnsignedTransaction::to_multisig_tx()`.
- #3035 **Breaking change (wire, client code)**: `TxDetails` replaces `chain_id` with `chain_hash_fragment`, a `u64` fragment of the target chain hash. This allows nodes to deterministically select the chain hash a transaction was signed over when multiple chain-hash overrides are active, instead of verifying by trial and error. V1 signing flows now also verify that all signatures are collected over the same chain hash.
- #2751 **Breaking change (consensus)**: The `Generation` uniqueness (replay protection) mechanism now uses non-malleable transaction hashes, fixing a hash malleability vulnerability with V1 transactions (see the operator migration step above for rollups with prior V1 traffic).
- #3032 **Breaking change (consensus)**: Generation and windowed-nonce uniqueness values that jump too far ahead of the last observed value are rejected: generations above `max(2 * last, last + PAST_TRANSACTION_GENERATIONS + 1)` and window nonces above `max(2 * highest_seen, PAST_TRANSACTIONS_WINDOW)`. This protects against user error irreversibly ratcheting an account's generation or nonce window to an unusable value (or even u64::MAX).
- #2813 **Breaking change (wire, client code)**: The Solana-offchain authenticator's V0 JSON payload now includes an explicit `"version": 0` field, aligning V0 with V1 and with the standard authenticator's version discriminator.

### Gas and EVM
- #2890 #2899 **Breaking change (code, consensus)**: Borsh deserialization is now metered by work done, via `MeteredReader` + `MeteredBorshDeserialize` (blanket-impl'd for `T: BorshDeserialize`), replacing the previous flat per-type charges (see the constants.toml delta above); a redundant extra gas charge is also removed.
- #2904 **Breaking change (code)**, EVM rollups only: receipt `effectiveGasPrice` and projected `gasUsed` are now unconditionally derived from the actual metered fee. The `EVM_RECEIPT_ACTUAL_FEE_HEIGHT` activation constant is removed; the new behavior is mandatory.
- #2934 **Breaking change (code)**: The block gas limit is now a single `BLOCK_GAS_LIMIT` constant for all heights, removing the `CHANGE_GAS_LIMIT_AFTER_HEIGHT` fork mechanism, the `sequencer.height_for_gas_limit_computation` config field, and the `ChainStateCapability::block_gas_limit` / `ChainState::block_gas_limit{,_at}` methods (use `<S as GasSpec>::block_gas_limit()` instead; test override env var `SOV_TEST_CONST_OVERRIDE_INITIAL_GAS_LIMIT` -> `SOV_TEST_CONST_OVERRIDE_BLOCK_GAS_LIMIT`). The per-block sequencer safeguards remain unconditionally enabled.
- #3037 **Breaking change (consensus)**, EVM rollups only: the enabled-custom-precompiles configuration is now unconditionally read from state during EVM execution, so gas depends only on the stored enabled set and never on the precompiles compiled into the binary. Custom precompiles can therefore be added or removed in a binary upgrade without forking execution, at the cost of one metered state read per EVM transaction.

### Other breaking changes
- #2948 Hyperlane: `IGPMetadata` deserialization now validates the `variant` field (must be `1`) and decodes the gas limit as a full 256-bit value with an overflow check; buffers may omit the trailing refund-address/custom-metadata bytes. Fixes audit finding SOV-2; `msg.value` and `refundAddress` remain intentionally ignored.
- #2741 #2908 *Minor breaking change (code)*: `UnsignedTransaction`'s redundant owned `call()` getter is removed (use `runtime_call()`), and the `resolve_context` authorization-capability signature is tightened to reflect that context resolution is now read-only.
- #3034 **Breaking change (code)**: Refactors Runtime traits to make custom sequencing data support more ergonomic: `HasSequencingData` is now independent of `HasCapabilities`, sequencing data is only populated for preferred-sequencer batches (no per-usage gating needed), and access is key-based with SDK-side access recording — custom data types only implement `get()` and `prune_to_used_keys()`. This considerably simplifies correct implementation of custom EVM precompiles that consume sequencing data. As a side-effect, rollups that don't use sequencing data must now define
```
...
# 2026-06-24
- #2994 Celestia adapter: updates the approximate DA block time from 6s to 3s ahead of the upcoming Celestia upgrade (`get_approximate_block_time`). The preferred sequencer's PI controller derives its rates from this value, so it self-scales — the tx acceptance rate roughly doubles and batches close about twice as often. Re-derives the RPC timeout defaults to the new block time (`request_timeout_secs` 38→20, `api_request_timeout_secs` 8→5) and halves the block-time-coupled values in the Celestia example configs (`da_polling_interval_ms` 2000→1000, `batch_execution_time_limit_millis` 12000→6000, `blob_processing_timeout_secs` 600→300) to keep wall-clock behavior aligned. Not state- or API-breaking; nodes that set these values explicitly are unaffected.
  * Block-count tolerances are intentionally left unchanged — in particular the consensus constant `DEFERRED_SLOTS_COUNT`. Because DA blocks now arrive twice as fast, the worst-case wall-clock tolerance to DA stalls / non-inclusion before the preferred sequencer recovers or shuts down is roughly halved (~5 min instead of ~10 min). Restoring the old wall-clock window would require raising `DEFERRED_SLOTS_COUNT`, a separate breaking change.
...
- #2989 Celestia adapter: the ZK verifier now rejects blobs whose witness-recorded total length does not match the `sequence_length` of the authenticated first share (`BlobDataError::MismatchedBlobLength`). Previously this prover-supplied value was never cross-checked, so a malicious prover could forge `total_len()` — which feeds gas accounting and size gates in the STF — while still producing a valid DA proof. Also introduces internal `compressed_*`/`rollup_*` accessors on `BlobWithSender` separating DA from rollup-visible byte accounting, in preparation for blob compression.
- #2989 Celestia adapter: adds the compressed-blob envelope (v1) parser/classifier (new `envelope` module: magic detection, fixed 24-byte header parsing, codec and per-chunk decompression-cap checks) and a serde-skipped `OnceLock` envelope cache on `BlobWithSender` (ignored by `PartialEq`, reconstructed deterministically after deserialization).
- #2989 **Breaking Change** Adds `BlobReaderTrait::rollup_decode_failed` (defaulted `false`) and restructures `sov-blob-storage`'s blob-acceptance gate to consult it: when an adapter's authenticated DA bytes are fully present but do not decode into the claimed rollup payload the sender is slashed, whereas a prover that exposes a valid prefix while withholding the tail fails closed. Identity adapters keep the default and are unaffected; only transforming adapters (Celestia compression) override it. Provers and full nodes must run the matching binary.
- #2989 Celestia adapter: wires transparent, config-gated LZ4 compression for batch blobs (`compression`, default `Off`; `compression_chunk_size`, default 482, validated to `1..=16384` at startup). The per-chunk verifier caps live in the root `constants.toml` — `CELESTIA_MAX_ROLLUP_CHUNK_LEN` (16 KiB uncompressed, the decompression-bomb bound) and `CELESTIA_MAX_COMPRESSED_CHUNK_SIZE` (1394 B, ≤ three signed shares, bounding the shares proven per chunk in ZK) — so an operator can tune them without an SDK release; they are consensus-relevant and compiled into both native and guest binaries. Blobs are posted as a chunked, forward-only-decodable envelope so verifier work still scales with bytes consumed. The LZ4 chunks are linked-block — each is compressed against the rolling 64 KiB dictionary window of the preceding decoded output — so cross-chunk redundancy is exploited and the small-chunk ratio approaches whole-blob LZ4 (≈60% fewer on-DA bytes than independent per-chunk compression on real batches), at the cost of in-order rather than random-access chunk decoding. Legacy raw blobs are unchanged and proofs always post verbatim. Emission only — read/verify semantics never depend on it; adds compression-ratio metrics.
