...
# 2026-06-12
- #2977 Celestia adapter: the ZK verifier now rejects blobs whose witness-recorded total length does not match the `sequence_length` of the authenticated first share (`BlobDataError::TotalLenMismatch`). Previously this prover-supplied value was never cross-checked, so a malicious prover could forge `total_len()` — which feeds gas accounting and size gates in the STF — while still producing a valid DA proof. Also introduces internal `compressed_*`/`logical_*` accessors on `BlobWithSender` separating DA-physical from rollup-visible byte accounting, in preparation for blob compression. No other behavior change.
# 2026-06-15
- #PR_NUMBER Celestia adapter: add the compressed-blob envelope (v1) header parser/classifier (new `envelope` module: magic detection, fixed 24-byte header parsing, codec and decompression-cap checks) and a serde-skipped `OnceLock` envelope cache on `BlobWithSender` (ignored by `PartialEq`, reconstructed deterministically after deserialization). Plumbing for upcoming blob compression — no change to live blob read/verify/submit behavior.

# 2026-06-12
# Celestia Adapter Compression
- #2977 Celestia adapter: the ZK verifier now rejects blobs whose witness-recorded total length does not match the `sequence_length` of the authenticated first share (`BlobDataError::TotalLenMismatch`). Previously this prover-supplied value was never cross-checked, so a malicious prover could forge `total_len()` — which feeds gas accounting and size gates in the STF — while still producing a valid DA proof. Also introduces internal `compressed_*`/`logical_*` accessors on `BlobWithSender` separating DA-physical from rollup-visible byte accounting, in preparation for blob compression. No other behavior change.
- #2978 Celestia adapter: add the compressed-blob envelope (v1) header parser/classifier (new `envelope` module: magic detection, fixed 24-byte header parsing, codec and decompression-cap checks) and a serde-skipped `OnceLock` envelope cache on `BlobWithSender` (ignored by `PartialEq`, reconstructed deterministically after deserialization). Plumbing for upcoming blob compression — no change to live blob read/verify/submit behavior.
# 2026-06-15
- #PR_NUMBER Celestia adapter: add the compressed-blob envelope (v1) header parser/classifier (new `envelope` module: magic detection, fixed 24-byte header parsing, codec and decompression-cap checks) and a serde-skipped `OnceLock` envelope cache on `BlobWithSender` (ignored by `PartialEq`, reconstructed deterministically after deserialization). Plumbing for upcoming blob compression — no change to live blob read/verify/submit behavior.

# 2026-06-12
- #2977 Celestia adapter: the ZK verifier now rejects blobs whose witness-recorded total length does not match the `sequence_length` of the authenticated first share (`BlobDataError::TotalLenMismatch`). Previously this prover-supplied value was never cross-checked, so a malicious prover could forge `total_len()` — which feeds gas accounting and size gates in the STF — while still producing a valid DA proof. Also introduces internal `compressed_*`/`logical_*` accessors on `BlobWithSender` separating DA-physical from rollup-visible byte accounting, in preparation for blob compression. No other behavior change.
- #PR_NUMBER **Breaking Change** Celestia adapter: the zk witness no longer serializes unread Celestia shares. `BlobWithSender` now serializes a pruned payload reader (the verified payload prefix plus its total length) instead of the full `BlobIterator`/share source, so a partially-read blob's witness is proportional to bytes actually read rather than to the blob's full on-DA size. The verifier also rejects witnesses whose verified prefix exceeds the claimed total length. The serialized form of `RelevantBlobs<BlobWithSender>` changes — nodes and provers must be upgraded together. A blob restored from a witness exposes only its verified bytes and cannot be advanced beyond them.
- #PR_NUMBER Celestia adapter: the ZK verifier now rejects blobs whose witness-recorded total length does not match the `sequence_length` of the authenticated first share (`BlobDataError::TotalLenMismatch`). Previously this prover-supplied value was never cross-checked, so a malicious prover could forge `total_len()` — which feeds gas accounting and size gates in the STF — while still producing a valid DA proof. Also introduces internal `compressed_*`/`logical_*` accessors on `BlobWithSender` separating DA-physical from rollup-visible byte accounting, in preparation for blob compression. No other behavior change.
# Celestia Adapter Compression
- #2977 Celestia adapter: the ZK verifier now rejects blobs whose witness-recorded total length does not match the `sequence_length` of the authenticated first share (`BlobDataError::TotalLenMismatch`). Previously this prover-supplied value was never cross-checked, so a malicious prover could forge `total_len()` — which feeds gas accounting and size gates in the STF — while still producing a valid DA proof. Also introduces internal `compressed_*`/`logical_*` accessors on `BlobWithSender` separating DA-physical from rollup-visible byte accounting, in preparation for blob compression. No other behavior change.
- #2978 Celestia adapter: add the compressed-blob envelope (v1) header parser/classifier (new `envelope` module: magic detection, fixed 24-byte header parsing, codec and decompression-cap checks) and a serde-skipped `OnceLock` envelope cache on `BlobWithSender` (ignored by `PartialEq`, reconstructed deterministically after deserialization). Plumbing for upcoming blob compression — no change to live blob read/verify/submit behavior.
# 2026-06-15
...
- #2977 Celestia adapter: the ZK verifier now rejects blobs whose witness-recorded total length does not match the `sequence_length` of the authenticated first share (`BlobDataError::TotalLenMismatch`). Previously this prover-supplied value was never cross-checked, so a malicious prover could forge `total_len()` — which feeds gas accounting and size gates in the STF — while still producing a valid DA proof. Also introduces internal `compressed_*`/`logical_*` accessors on `BlobWithSender` separating DA-physical from rollup-visible byte accounting, in preparation for blob compression. No other behavior change.
- #2978 Celestia adapter: add the compressed-blob envelope (v1) header parser/classifier (new `envelope` module: magic detection, fixed 24-byte header parsing, codec and decompression-cap checks) and a serde-skipped `OnceLock` envelope cache on `BlobWithSender` (ignored by `PartialEq`, reconstructed deterministically after deserialization). Plumbing for upcoming blob compression — no change to live blob read/verify/submit behavior.
- #2989 Celestia adapter: `compression_chunk_size` is now validated at startup — a value outside `1..=1446` (the verifier's per-chunk cap, `MAX_LOGICAL_CHUNK_LEN`) is rejected with a clear error instead of being silently clamped. The per-chunk cap is raised from 482 to 1446 (three continuation shares) so advanced operators can opt into larger chunks for a better compression ratio; the default chunk size stays 482 (one continuation share). Larger chunks reduce envelope framing overhead at the cost of coarser partial-read granularity. Verifier work still scales with compressed bytes consumed (chunk-granular).
