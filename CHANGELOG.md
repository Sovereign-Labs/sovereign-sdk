...
- #PR_NUMBER **Breaking Change** Celestia adapter: the zk witness no longer serializes unread Celestia shares. `BlobWithSender` now carries a pruned `BlobPayloadClaim` (the verified payload prefix plus its total length) across the witness boundary instead of the full `BlobIterator`/share source, so a partially-read blob's witness is proportional to bytes actually read rather than to the blob's full on-DA size. The serialized form of `RelevantBlobs<BlobWithSender>` changes — nodes and provers must be upgraded together. A blob restored from a witness exposes only its verified bytes and cannot be advanced beyond them.
