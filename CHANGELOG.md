...
# 2026-06-02
- #2886 **Breaking config change**: Replaces the flat `storage.pruner_block_interval` / `pruner_versions_to_keep` / `pruner_max_batch_size` fields with a single `storage.pruner` enum. Modes: omit the section (or `pruner = "off"`) to disable pruning (the default); `[storage.pruner.periodic]` with `block_interval` + `versions_to_keep` (+ optional `max_batch_size`) for the previous interval-driven background pruning; and `[storage.pruner.once_at_startup]` with `versions_to_keep` (+ optional `max_batch_size`, `compact_after`) to prune synchronously to completion once at node startup — before block processing begins — and then never again that run, so the pruning I/O does not compete with live read/write traffic. With `compact_after = true`, the accessory state column family and the user/kernel archival historical + pruning column families are compacted after the startup prune to drop tombstones and reclaim disk (heavy; off by default). Existing TOML configs must move to the new `[storage.pruner...]` form. Bumps the `rockbound` dependency to rev `fa798921e994de7c487e9fe022ea68f6d29602fd`, which makes the user/kernel pruning emit a range-delete tombstone for the pruning column family (instead of per-key point deletes), adds `VersionedDB::trigger_compaction` so `compact_after` compacts the user/kernel archival CFs, and makes `SchemaBatch::merge` preserve range operations.

# 2026-05-27
- #2886 Re-enables NOMT state-version pruning (disabled since the rockbound type-aware refactor). Bumps the `rockbound` dependency to rev `562187a1515ae55af6126891d20537806cfb8527`, which owns the historical/pruning column families and exposes the `collect_pruning_batch` API the pruner now uses. Hardens the native unversioned read path in `sov-state`: unbound NOMT reads now log and return `None` on a transient I/O fault instead of `.expect()`-panicking (by construction they can never observe a pruned version), and the rare pruned-version-race panic on a stale `Storage::get` reader is now preceded by a `tracing::warn!` so it is observable in production logs. The mockda→celestia migration enables periodic pruning for migrated nodes that don't configure it (see the `2026-06-02` entry for the `storage.pruner` config that supersedes the original flat `pruner_*` fields).
