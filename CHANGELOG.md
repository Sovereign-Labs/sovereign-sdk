...
* A limit on generation and window-based uniqueness nonce growth, which now reject generations or nonces that are larger than 2x the last used generation/nonce as a sanity check against user error (as these mechanisms are racheting, and unlimited increases made it trivial to accidentally burn an account).
* A refactor for custom EVM precompiles, which makes correct precompile implementation much simpler for developers.

Upgrading will require adjustments to SDK breaking changes in rollup and client code, changes to client logic where appropriate (especially around `sov-account` handling), and an operational upgrade with a state migration.

### Operating across a hard fork
Major hard forks of Sovereign SDK require operating separate rollup binaries pre- and post-fork. The rollup blueprint accepts `stop_at_rollup_height` and `start_at_rollup_height` parameters, conventionally exposed through the CLI as `--stop-at-rollup-height` and `--start_at_rollup_height`; to upgrade, a fork height (in the future) must be selected, and the old rollup binary restarted with the upgrade height passed in. State migrations must then be ran at this point to prepare for the new version. The new rollup binary can then be started from the corresponding start block (the SDK provides a utility for this). To automate this, we provide the `sov-rollup-manager` utility which wraps the running rollup process, and takes a list of paths to binaries and the configured start-stop heights as configuration.
...
- #2773 **Breaking change (code, wire, consensus)**: `sov_accounts::CallMessage` gains three variants — `AddCredentialToAddress { address, credential }`, `RemoveCredentialFromAddress { address, credential }`, and `RotateCredentialOnAddress { address, old_credential, new_credential }` — and is now generic in `S: Spec`. All three new variants require `context.sender() == address` (callers can only modify credentials on the address they are currently signing as); `InsertCredentialId` semantics are unchanged. `Rotate` is functionally equivalent to a `Remove` + `Add` collapsed into a single call for key rotation. There is no orphan guard on remove.
...
- #2688 **Breaking change(code, state, consensus)**: The transaction formats have changed in this version. This is a consensus breaking change and requires coordinating an upgrade using `--stop-at-rollup-height`, and using sov-rollup-manager for full resyncs from this point onwards for existing rollups. This fixes Credential ID malleability that allowed the same set of signed bytes to be replayed for different credentials, across V0 and V1 (multisig) transactions and with different V1 multisig parameters, creating a footgun that could cause security issues if accounts were not separated properly. With the malleability eliminated, signing key reuse across different multisigs is no longer a concern.
- #2864 **Breaking change(code, consensus)**: Hard fork version tracking: a new `chain_state.state_version` state item has been added, and a corresponding `STATE_VERSION` entry in constants.toml. These must match for the rollup to start. The binary `STATE_VERSION` will be incremented on breaking hard forks. On genesis, the state value must be initialised to the starting rollup binary's version, and it will subsequently be incremented by migrations between versions. Pre-fork rollup builds are implicitly considered version `0`; this hard fork increments the state version to `1`, so rollup binaries after the hard fork must set `STATE_VERSION = 1` in their constants.toml.
