...
- #2196 *Minor breaking change (code)*: `MetricsTracker::submit_inline` has been removed. All previously-inline metrics (`sov_rollup_num_of_in_flight_blobs`, `sov_rollup_blobs_enter_scope`, `sov_rollup_blobs_exit_scope`, `sov_rollup_current_sequence_number`, `sov_rollup_in_progress_batch_size`, `sov_rollup_sequence_number_delta`) now have dedicated types implementing `Metric`; their line-protocol output is unchanged, so dashboards keep working. If you were calling `submit_inline` from outside the SDK, define a struct implementing the `Metric` trait and call `tracker.submit(...)` instead.
