...
| `sov_db_pruner` | counter + timer | `sov-db/src/metrics/nomt.rs` | One point per database per pruner pass; tag `db` splits `user`/`kernel` (historical state) from `ModuleAccessoryState` (accessory). `keys_to_prune` near zero while `keys_inspected` is non-zero = wasted scans (retention misconfigured?). No emissions over long windows = pruner task may be stuck. `time_us` is the scan/collect cost only — the write cost is `sov_storage_manager_finalization.pruning_commit_time`. |
| `sov_db_compaction` | timer | `sov-db/src/metrics/nomt.rs` | Post-prune RocksDB compaction time per DB (`accessory_us` / `user_us` / `kernel_us` / `total_us`). Only emitted by the one-time startup prune (`OnceAtStartup` + `compact_after`), so it is sparse — at most one point per node start. Large `total_us` = long blocking startup compaction. |
