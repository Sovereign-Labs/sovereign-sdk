...
pub trait DbBackend: Send + Sync + 'static {
    ...
    async fn relinquish_leadership_on_shutdown(&mut self) -> Result<()> {
        Ok(())
    }
}
...
impl PreferredSequencerDb {
    ...
    pub(crate) async fn relinquish_leadership_on_shutdown_best_effort(&mut self) {
        const SHUTDOWN_RELINQUISH_TIMEOUT: Duration = Duration::from_millis(500);
        ...
        match tokio::time::timeout(
            SHUTDOWN_RELINQUISH_TIMEOUT,
            backend.relinquish_leadership_on_shutdown(),
        )
        .await
        {
            Ok(Ok(())) => {}
            Ok(Err(error)) => {
                tracing::warn!(
                    error = ?error,
                    "Failed to mark leadership relinquished on shutdown; failover will use timeout-based election."
                );
            }
            Err(_) => {
                tracing::warn!(
                    timeout_ms = SHUTDOWN_RELINQUISH_TIMEOUT.as_millis() as u64,
                    "Timed out marking leadership relinquished on shutdown; failover will use timeout-based election."
                );
            }
        }
    }
