    ...
    Da: DaService<Spec = S::Da>,
{
    ...
    pub async fn build(
        ...
        let (next_sequence_number, mut db_cache) = db.initial_data().await?;

        // After a restart, the DB may contain proof blobs with sequence numbers
        // the kernel has already advanced past. Cap the counter at the kernel's
        // expected value and prune stale cache entries that would collide with
        // the reset counter (sovereign-labs/sovereign-sdk#2558).
        let node_sequence_number =
            get_next_sequence_number_according_to_node(&latest_state_update, &mut Rt::default());
        let next_sequence_number =
            if next_sequence_number > node_sequence_number && node_sequence_number > 0 {
                tracing::info!(
                    db_sequence_number = next_sequence_number,
                    node_sequence_number,
                    "Resetting sequencer sequence counter to match kernel after restart"
                );
                // Remove proof entries from the cache — they have stale sequence
                // numbers from the previous run that would collide with fresh
                // numbers assigned after the counter reset. Batches are kept since
                // they're needed for replay. Old proof blobs are already on DA —
                // new ones will be generated by the ZK proof manager.
                db_cache.remove_all_proofs();
                node_sequence_number
            } else {
