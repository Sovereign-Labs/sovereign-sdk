...
/// Overlapping subnets resolve by longest prefix: a nested /24 (generous)
/// overrides the enclosing /8 (zero) for IPs it contains, while IPs only in the
/// /8 stay blocked.
#[tokio::test(flavor = "multi_thread")]
async fn test_overlapping_subnets_use_longest_prefix() {
    let genesis = Genesis::new();
    let sov_config = SovRateLimiterConfig {
        max_requests_per_second: 1000,
        max_nb_of_concurrent_users_in_rate_limiter: 1000,
        // Generous defaults so the address limiter never interferes.
        default_limits: Limits {
            resources_per_bucket: 100,
            refill_rate: 100,
        },
        height_for_gas_limit_computation: RollupHeight::GENESIS,
        address_custom_limits: Vec::default(),
    ...
    // IP inside the nested /24 -> longest-prefix /24 (generous) -> accepted.
    {
        let tx = tx_set_value_and_sleep(&admin.private_key, 0, 100, 0);
        let request = AcceptTx {
            body: sov_sequencer::rest_api::Base64Blob { blob: tx.data },
        };
        let resp = client
        ...
        assert!(
            ...
            "IP in the nested /24 should use the generous limit, got status {}",
            resp.status()
        );
    }

    // IP only in the broad /8 -> /8 (zero) -> rejected.
    {
        let key = <<TestSpec as Spec>::CryptoSpec as CryptoSpec>::PrivateKey::generate();
        let tx = tx_set_value_and_sleep(&key, 0, 100, 0);
        let request = AcceptTx {
        ...
        let err_str = std::str::from_utf8(&body).unwrap();
        assert!(
            err_str.contains("Resource limit exceeded for IP: 10.0.1.5"),
            "IP only in the broad /8 should be blocked by its zero limit, got: {err_str}"
        );
    }
}
