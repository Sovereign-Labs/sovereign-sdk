...
async fn create_test_nomt_rollup() -> (TestRollup<TestNomtBlueprint>, TestUser<TestSpec>) {
    ...
    .set_da_config(|c| {c.sender_address = seq_da_address; c.finalization_blocks = 10;})
    .set_persistent_da()
...
async fn send_value_and_update_values(test_rollup: &TestRollup<TestNomtBlueprint>, user: &TestUser<TestSpec>, key: (HexHash, u32), new_value: Option<u32>, values: &mut BTreeMap<StateKey, u32>, iter: u32) {
	let tx = tx_modify_state_map(
		&user.private_key,
		iter as u64,
		key.0,
		key.1,
	...
	test_rollup.api_client().accept_tx(&api_types::AcceptTxBody {
		body: BASE64_STANDARD.encode(&tx),
	}).await.unwrap();
	if let Some(new_value) = new_value {
		values.insert(StateKey { address: key.0, index: key.1 }, new_value);
	} else {
		values.remove(&StateKey { address: key.0, index: key.1 });
	}
}
...
async fn do_step(da_layer: &mut DaLayerWithSubscription, test_rollup: &TestRollup<TestNomtBlueprint>, user: &TestUser<TestSpec>, key: (HexHash, u32), new_value: Option<u32>, values: &mut BTreeMap<StateKey, u32>, iter: u32) {
...
async fn test_state_map_iteration() {
	...
	for i in 0.. 100 {
		...
		// This little block uses a permutation of 0..255 to generate a sequence of insertions and deletions.
		// We do 2 insertions and 1 deletion per group of 3 operations.
		...
		do_step(&mut da_layer, &test_rollup, &user, (address, index), value, &mut values, i).await;
	}
...
impl<K, V> StateItemContents<K, V> {
	...
	pub fn value(&self) -> Option<&V> {
		match self {
			StateItemContents::MapElement { value, .. } => Some(value),
			_ => None,
		}
	}
}
