...
use crate::metrics::{track_finalized_ws_replay, LedgerWsConnectionGuard};
        ...
        + 'static,
{
    ...
    async fn subscribe_to_slot_events(
        ...
        ws.on_upgrade(|socket| async move {
            let _connection_guard = LedgerWsConnectionGuard::slot_events();
    ...
    async fn subscribe_to_aggregated_proofs(
        ...
        ws.on_upgrade(|socket| async move {
            let _connection_guard = LedgerWsConnectionGuard::aggregated_proofs();
    ...
    async fn subscribe_to_head(
        ...
        ws.on_upgrade(move |socket| async move {
            let _connection_guard = LedgerWsConnectionGuard::head(query_mode);
    ...
    async fn subscribe_to_finalized(
        ...
        ws.on_upgrade(move |socket| async move {
            let _connection_guard = LedgerWsConnectionGuard::finalized(query_mode);
                ...
                .scan(
                    ...
                    move |last_notified_slot, incoming_slot_num| {
                        ...
                        *last_notified_slot = incoming_slot_num.saturating_add(1);
                        let replayed_slots =
                            incoming_slot_num.saturating_delta(old_last).saturating_add(1);
