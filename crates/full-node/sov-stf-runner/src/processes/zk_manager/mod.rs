    ...
    Ps::DaService: DaService<Error = anyhow::Error>,
{
    ...
    pub async fn post_aggregated_proof_to_da_in_background(self) -> JoinHandle<()> {
        tokio::spawn(async move {
            ...
            // Wrap the whole task in `future_or_shutdown` so a shutdown signal
            // interrupts it at *any* await point — including a long-running
            // proof generation step. This avoids having to thread shutdown
            // checks through every step of the proving op.
            let aggregator_shutdown = self.shutdown_receiver.clone();
            let aggregator_handle = tokio::spawn(async move {
                match future_or_shutdown(aggregator.run(), &aggregator_shutdown).await {
            ...
            let intake_shutdown = self.shutdown_receiver;
            ...
            match future_or_shutdown(intake.run(), &intake_shutdown).await {
                FutureOrShutdownOutput::Shutdown => {
    ...
    Ps::DaService: DaService<Error = anyhow::Error>,
{
    async fn run(mut self) -> anyhow::Result<()> {
        ...
        // Shutdown is handled by the caller, which wraps this whole future in
        // `future_or_shutdown` and cancels it at the current await point.
        loop {
    ...
    Ps::DaService: DaService<Error = anyhow::Error>,
{
    async fn run(mut self) -> anyhow::Result<()> {
        loop {
            // Shutdown is handled by the caller, which wraps this whole future
            // in `future_or_shutdown` and cancels it at the current await point.
