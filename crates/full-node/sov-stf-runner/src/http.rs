...
async fn handle_socket(socket: WebSocket, rpc_methods: RpcModule<()>) {
    ...
    tokio::spawn(handle_socket_write(socket_responses, sender, close_receiver));
    tokio::spawn(handle_socket_read(receiver, socket_requests, rpc_methods, close_sender));
}
