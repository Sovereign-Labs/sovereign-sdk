    ...
    Rt: Runtime<S> + DispatchCall<Spec = S>,
{
    ...
    > {
        ...
        authenticate::<_, S, Rt, Rt>(&tx.data, state)
    }
    ...
    > {
        ...
        Ok(authenticate::<_, S, Rt, Rt>(&tx.data, state)?)
    }
...
>(
    ...
    verify_and_decode_tx::<S, D, Rt>(raw_tx_hash, tx, raw_tx, state)
}
...
>(
    ...
    match &tx.versioned_tx {
        VersionedTx::V0(tx_v0) => {
            ...
            verify_eip712_signature::<S, D, Rt>(&tx, raw_tx_hash, raw_tx_bytes, meter)?;
...
>(
    ...
    // Use the schema to calculate the EIP712 signing hash
    let schema = Rt::schema();
    let transaction_type_index = schema.rollup_expected_index(sov_modules_api::sov_universal_wallet::schema::RollupRoots::Transaction)
        .map_err(|e| AuthenticationError::FatalError(
            FatalError::SigVerificationFailed(format!("Cannot verify EIP712 signature. Failed to get transaction type from schema: {e}")),
            raw_tx_hash,
        ))?;

    let eip712_hash = schema
        .eip712_signing_hash(transaction_type_index, raw_tx_bytes)
        .map_err(|e| {
