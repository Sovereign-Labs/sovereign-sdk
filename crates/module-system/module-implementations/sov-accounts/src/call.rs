...
pub enum CallMessage {
    /// Registers `credential_id` as an authorized signer whose transactions
    /// execute as the caller's address. Fails if the credential is already
    /// mapped to any address in this module (credentials are globally unique).
    InsertCredentialId(
