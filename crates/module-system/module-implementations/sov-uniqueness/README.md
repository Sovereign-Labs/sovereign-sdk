...
The module does not expose any `CallMessage` therefore, its state can't be directly modified by the users of the rollup. Instead the nonces/generation/height buckets are modified via the rollup's capabilities.
...
- Height deduplication: Each transaction sent by a given `sov_rollup_interface::crypto::CredentialId` can provide a rollup height. The transaction is accepted while `current_rollup_height - tx_height <= PAST_TRANSACTION_HEIGHTS`, and each height bucket deduplicates by transaction hash. Buckets older than the cutoff are pruned when transactions are marked attempted.
