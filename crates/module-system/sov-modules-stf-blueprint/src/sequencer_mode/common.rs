...
use sov_modules_api::capabilities::{
    calculate_hash, HasCapabilities, SequencingDataHandler, TimelockCapability,
};
...
fn attempt_tx<S: Spec, RT: Runtime<S>, I: StateProvider<S>>(
    ...
    if let Some(policy) = runtime.timelock_for_callmessage(&message) {
        let proposal_id = calculate_hash::<S>(&RT::encode(&message));
        let proposal_exists = {
        ...
        if proposal_exists {
            {
                ...
                timelock
                    .try_unlock_proposal(ctx.sender(), &proposal_id, state)
