...
impl BucketStorage {
    ...
    pub fn try_insert(&mut self, key: SlotKey, value: SlotValue) -> bool {
        let size: usize = value
            .size()
            .try_into()
            .expect("Unable to cast u32 to usize. This is impossible on 64-bit platforms.");
    ...
    pub fn delete(&mut self, key: &SlotKey) {
        ...
        let size: usize = size
            .try_into()
            .expect("Unable to cast u32 to usize. This is impossible on 64-bit platforms.");
        self.current_size.checked_sub(size).expect(
            "Bucket size underflowed! This is a bug in the implementaiton of the pinned cache.",
        );
    }
}
...
impl PinnedCache {
    ...
    pub fn try_load_bucket_if_absent<S: NativeStorage>(
        &mut self,
        bucket_id: BucketId,
        storage: &S,
        max_size: usize,
        ...
        let key_length = bucket_id
            .0
            .len()
            .checked_sub(2)
            .ok_or_else(|| anyhow::anyhow!("Bucket ID is too short to have a prefix"))?;

        let state_item_cache =
            self.item_caches
                .entry(bucket_id.0.prefix())
                .or_insert(StateItemCache {
                    bucket_key_length: key_length,
        ...
        tracing::debug!(
            "PinnedCache: Loaded bucket {} with {} items. Size: {}",
            bucket_id.0,
