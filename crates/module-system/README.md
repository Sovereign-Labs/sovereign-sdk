...
its `Storage` type will almost certainly be `NomtProverStorage`, which holds its data in NOMT-backed native state storage.
But if you're running in zk mode the `Storage` type will instead be `NomtVerifierStorage`, which reads its data from a set of
"hints" provided by the prover. Because all the rollups modules are generic, none of them need to worry about this distinction.
