...
impl<S: Spec> UniversalStateAccessor for ApiStateAccessor<S> {
    fn get_size(
        ...
        let result = match namespace {
            Namespace::User => if let Some(number) = self.safe_true_slot_number_to_use {
            ...
            // FIXME: Handle kernel and accessory cases.
    ...
    fn get_value(
        ...
        let result = match namespace {
            ...
            Namespace::Kernel => if let Some(_) = self.safe_true_slot_number_to_use {
...
impl<S: Spec + 'static> ApiStateAccessor<S> {
    ...
    fn new_with_price_and_state_to_access(
        ...
        let mut out = Self {
            ...
            intermediate_state: delta.intermediate_state.as_ref().map(|g| g.box_clone()),
