...
impl<S: Spec> UniversalStateAccessor for ApiStateAccessor<S> {
    ...
    fn get_value(
        ...
        let result = match namespace {
            Namespace::User => {
                if let Some(number) = self.safe_true_slot_number_to_use {
                    let archival_result = self.get_archival_from_user_storage(key, Some(number));
...
impl<S: Spec> ApiStateAccessor<S> {
    ...
    fn get_archival_from_user_storage(
        ...
        if is_evm {
            eprintln!(
                "[DIAG] get_archival_from_user_storage for key '{}' START",
                hex_key,
            );
        }
        if let Some(entry) = self.local_user_writes.get(key) {
            if is_evm {
                match entry.as_ref() {
                    None => {
                        eprintln!("[DIAG] get_archival_from_user_storage for key '{}' found None in local_writes", hex_key);
                    }
                    Some(v) => {
                        eprintln!("[DIAG] get_archival_from_user_storage for key '{}' found {} in local_writes", hex_key, hex::encode(v.value()));
                    }
                }
            }
            if is_evm {
                eprintln!("[DIAG] get_archival_from_user_state DONE");
            }
                ...
                MaybePresentValue::Present(present_uncommited_value) => {
                    if is_evm {
                        eprintln!(
                            ...
                            hex_key,
                            present_uncommited_value.map(|v| hex::encode(v.value()))
                        );
                    }
                }
                MaybePresentValue::Absent => {
                    if is_evm {
                        eprintln!("[DIAG] key '{}' absent from uncommited changes", hex_key);
                    }
                }
            }
        } else if is_evm {
            eprintln!("[DIAG] no uncommited changes for key '{}'", hex_key,);
        }
        ...
        if is_evm {
            eprintln!(
                "[DIAG] fetched from historical key '{}' value: {:?}",
                hex_key,
                historical.as_ref().map(|v| hex::encode(v.value()))
            );
        }
        if is_evm {
            eprintln!("[DIAG] get_archival_from_user_state DONE");
