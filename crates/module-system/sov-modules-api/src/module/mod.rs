...
use sov_state::{BorshCodec, EventContainer, Prefix, SlotKey};
...
pub trait ModuleInfo {
    ...
    /// Returns a unique SlotKey generated by this module.
    fn module_key(&self) -> SlotKey {
        let prefix = self.prefix();
        let name = prefix.module_name().to_string();

        let prefix: Prefix = self.prefix().into();
        SlotKey::new::<_, String, _>(&prefix, &name, &BorshCodec {})
    }
