...
/// Versioned transaction signing payload. The borsh enum discriminant serves as the version byte
/// in signed bytes: `V0` = `0x00`, `V1` = `0x01`.
///
/// This is the type serialized to produce the bytes that
...
pub enum TransactionSigningPayload<R: TransactionCallable, S: Spec> {
    /// V0 (single-sig) signing payload.
    V0(
        #[borsh(bound(
            serialize = "R::Call: BorshSerialize",
            deserialize = "R::Call: BorshDeserialize",
        ))]
        TransactionSigningPayloadV0<R, S>,
    ),
    /// V1 (multisig) signing payload, includes credential commitment.
    V1(
        #[borsh(bound(
            serialize = "R::Call: BorshSerialize",
            deserialize = "R::Call: BorshDeserialize",
        ))]
        TransactionSigningPayloadV1<R, S>,
    ),
}

impl<R: TransactionCallable, S: Spec> Clone for TransactionSigningPayload<R, S> {
    fn clone(&self) -> Self {
        match self {
...
impl<R: TransactionCallable, S: Spec> TransactionSigningPayload<R, S> {
    ...
    /// Returns the signer-declared address override, if any.
    pub fn address_override(&self) -> Option<S::Address> {
        match self {
            Self::V0(v0) => v0.address_override,
            Self::V1(v1) => v1.address_override,
        }
    }

    /// Returns the chain hash committed to by this signing payload.
    pub fn chain_hash(&self) -> &[u8; 32] {
        match self {
            Self::V0(v0) => &v0.chain_hash,
            Self::V1(v1) => &v1.chain_hash,
        }
    }
}
