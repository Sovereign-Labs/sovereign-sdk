...
/// Machine-readable authentication failure codes.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum AuthenticationFailureCode {
    /// The transaction's `max_fee_per_gas` is lower than the rollup base fee.
    InsufficientMaxFeePerGas,
}

/// Structured authentication failure details that can be propagated across API layers.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct AuthenticationFailureDetails {
    /// Human-readable description preserved for debugging and client compatibility.
    pub error: String,
    /// Stable machine-readable error code, when available.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub code: Option<AuthenticationFailureCode>,
...
impl AuthenticationFailureDetails {
    ...
    fn from_fatal_error_with_message(error: &FatalError, message: String) -> Self {
        match error {
    ...
    /// Creates structured failure details from a fatal authentication error.
    pub fn from_fatal_error(error: &FatalError) -> Self {
        Self::from_fatal_error_with_message(error, error.to_string())
    }

    /// Creates structured failure details from a registered-sequencer authentication error.
    pub fn from_authentication_error(error: &AuthenticationError) -> Self {
        match error {
            AuthenticationError::FatalError(fatal_error, _) => {
                Self::from_fatal_error_with_message(fatal_error, error.to_string())
            }
            AuthenticationError::OutOfGas(_) => Self::new(error.to_string()),
        }
    }

    /// Creates structured failure details from an unregistered-sequencer fatal authentication error.
    pub fn from_unregistered_fatal_error(error: &FatalError) -> Self {
        Self::from_fatal_error_with_message(
            error,
            format!("Unregistered sequencer authentication failed: {error}"),
        )
    }

    /// Returns true if the failure corresponds to a low `max_fee_per_gas`.
    pub fn is_insufficient_max_fee_per_gas(&self) -> bool {
        matches!(
            self.code,
...
impl std::fmt::Display for AuthenticationFailureDetails {
...
mod tests {
    ...
    #[test]
    fn low_max_fee_fatal_error_produces_structured_details() {
        let details =
            AuthenticationFailureDetails::from_fatal_error(&FatalError::InsufficientMaxFeePerGas {
    ...
    #[test]
    fn auth_error_preserves_structured_details() {
        let details = AuthenticationFailureDetails::from_authentication_error(
            &AuthenticationError::FatalError(
                FatalError::InsufficientMaxFeePerGas {
                    user_max_fee_per_gas: 6,
                    rollup_base_fee: 7,
    ...
    #[test]
    fn unregistered_failure_details_keep_prefix() {
        let details = AuthenticationFailureDetails::from_unregistered_fatal_error(
            &FatalError::SigVerificationFailed("bad signature".to_string()),
        );

        assert_eq!(details.code, None);
        assert_eq!(
            details.error,
            "Unregistered sequencer authentication failed: Signature verification failed: bad signature"
        );
    }
}
