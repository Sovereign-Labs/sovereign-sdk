        ...
        BatchReceipt {
            ...
            tx_receipts: vec![
                TransactionReceipt::<TestTxReceiptContents> {
                    ...
                    body_to_save: Some(FullyBakedTx::new(b"tx0 body".to_vec())),
                    events: events(0),
                    receipt: TxEffect::Successful(0),
                },
                TransactionReceipt::<TestTxReceiptContents> {
                    tx_hash: sha2::Sha256::digest(b"tx1").into(),
                    body_to_save: Some(FullyBakedTx::new(b"tx1 body".to_vec())),
                    events: events(1),
        ...
        BatchReceipt {
            ...
            tx_receipts: vec![
                TransactionReceipt::<TestTxReceiptContents> {
                    ...
                    body_to_save: Some(FullyBakedTx::new(b"tx2 body".to_vec())),
                    events: events(2),
                    receipt: TxEffect::Successful(2),
                },
                TransactionReceipt::<TestTxReceiptContents> {
                    tx_hash: sha2::Sha256::digest(b"tx3").into(),
                    body_to_save: Some(FullyBakedTx::new(b"tx3 body".to_vec())),
                    events: events(3),
        ...
        BatchReceipt {
            ...
            tx_receipts: vec![
                TransactionReceipt::<TestTxReceiptContents> {
                    ...
                    body_to_save: Some(FullyBakedTx::new(b"tx4 body".to_vec())),
                    events: events(4),
                    receipt: TxEffect::Successful(4),
                },
                TransactionReceipt::<TestTxReceiptContents> {
                    tx_hash: sha2::Sha256::digest(b"tx5").into(),
                    body_to_save: Some(FullyBakedTx::new(b"tx5 body".to_vec())),
                    events: events(5),
...
fn batch3_tx_receipts() -> Vec<TransactionReceipt<TestTxReceiptContents>> {
    (0..260u64)
        .map(|i| TransactionReceipt::<TestTxReceiptContents> {
            ...
            body_to_save: Some(FullyBakedTx::new(format!("tx{} body", 6 + i).into_bytes())),
            events: events(6 + i),
