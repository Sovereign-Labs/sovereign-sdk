    ...
    N: Network + Send + Sync,
{
    ...
    for i in 1..=count {
        ...
        let (path, base_amount) = if is_usdc_to_weth {
            (
                vec![*contracts.usdc.address(), *contracts.weth.address()],
                "20000",
            ) // Max 20k USDC
        } else {
            (
                vec![*contracts.weth.address(), *contracts.usdc.address()],
                "10000",
            ) // Max 10k WETH
        };
    ...
    N: Network + Send + Sync,
{
    ...
    contracts
        .router
        .swapExactTokensForTokens(amount_in, expected_out, path, signer)
        .submit()
        .await?;
...
async fn deploy_uniswap_contracts<P, N>(client: &P) -> Result<Contracts<'_, P, N>>
where
    P: Provider<N> + Clone + Send + Sync,
    N: Network + Send + Sync,
{
