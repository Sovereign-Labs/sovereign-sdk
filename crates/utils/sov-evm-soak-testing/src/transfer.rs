...
pub(crate) async fn run(
    ...
    let priv_keys: Vec<_> = generate_priv_keys(count, faucet_private_key)?
        .into_iter()
        .map(|pk| (RpcClient::alloy_address(&pk), pk))
        .collect();

    // Send funds to all participants in the benchmark.
    let faucet_client = RpcClient::new(faucet_private_key, rpc_addr).await;
    let block_number = faucet_client.block_number().await;
    for (addr, _) in priv_keys.iter() {
        faucet_client.alloy_send_eth(*addr, 2).await;
    }
    ...
    loop {
        let current_block_number = faucet_client.block_number().await;
    ...
    let faucet_address = RpcClient::alloy_address(faucet_private_key);
    for (_, pk) in priv_keys.into_iter() {
        tokio::spawn(async move {
            let client = RpcClient::new(&pk, rpc_addr).await;
            client.alloy_send_eth(faucet_address, 1).await;
            dbg!(client.address());
        });
    }
