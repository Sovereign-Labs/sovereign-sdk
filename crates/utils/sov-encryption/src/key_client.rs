...
pub fn create_key_client(config: KeyClientConfig) -> Box<dyn KeyClient> {
    match config {
        ...
        KeyClientConfig::UnixSocket {
            ...
            retry_delay_ms,
            ..
        } => Box::new(UnixSocketKeyClient::new(
            socket_path,
            timeout,
            max_retries,
            retry_delay_ms,
...
impl StaticKeyClient {
    pub fn new(encryption_key: String, decryption_key: Option<String>) -> Self {
        let enc_key = hex::decode(&encryption_key).expect("Invalid hex encryption key");
...
impl UnixSocketKeyClient {
    ...
    ) -> Result<KeyResponse, EncryptionError> {
        let mut retries = 0;
    ...
    async fn try_request_key(
        ...
        let stream = timeout(self.timeout, UnixStream::connect(&self.socket_path))
            .await
            .map_err(|_| {
                EncryptionError::KeyServerConnection(std::io::Error::new(
    ...
    async fn send_request(
        ...
        let response: KeyResponse = serde_json::from_slice(&response_bytes).map_err(|e| {
            EncryptionError::KeyServerResponse(format!("Invalid JSON response: {}", e))
        })?;
...
mod tests {
    ...
    async fn test_static_key_client_key_id_ignored() {
        ...
        let key2 = client
            .get_decryption_key(Some("some_key_id".to_string()))
            .await
            .unwrap();
