...
- **State item ordering.** `#[state]` field prefixes are positional within a module: appending a new field is safe, but inserting or reordering shifts the storage keys of all following fields. Module discriminants are pinned in `constants.toml`. Chain identity (`CHAIN_ID`, `CHAIN_NAME`, `CHAIN_HASH_OVERRIDES`, DA namespaces) lives in `chain-metadata.toml`.
