...
impl<C: Context> Genesis for Runtime<C> {
    type Context = C;
    type Config = <C::Storage as Storage>::Config;

    fn genesis(
        config: Self::Config,
    ) -> Result<<<Self as Genesis>::Context as Spec>::Storage, Error> {
        let storage = C::Storage::new(config);
        let mut election = <Election<C> as sov_modules_api::ModuleInfo<C>>::new(storage.clone());
        election.genesis()?;
        let mut value_adder =
            <ValueSetter<C> as sov_modules_api::ModuleInfo<C>>::new(storage.clone());
        value_adder.genesis()?;
        Ok(storage)
    }
}
