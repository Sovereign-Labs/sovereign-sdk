...
fn main() { 
    ...
    {
        let result = <TestRuntime<MockContext> as TestStructRpcServer<MockContext>>::first_method(&runtime);
        assert_eq!(result.unwrap(), 11);
    }

    {
        let result = <TestRuntime<MockContext> as TestStructRpcServer<MockContext>>::second_method(&runtime, 22);
        assert_eq!(result.unwrap(), 22);
    }

    {
        let result = <TestRuntime<MockContext> as TestStructRpcServer<MockContext>>::third_method(&runtime, 33);
        assert_eq!(result.unwrap(), 33);
    }

    {
        let result = <TestRuntime<MockContext> as TestStructRpcServer<MockContext>>::fourth_method(&runtime, 44);
        assert_eq!(result.unwrap(), 44);
    }

    {
        let result = <TestRuntime<MockContext> as TestStructRpcServer<MockContext>>::health(&runtime);
